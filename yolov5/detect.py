...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='/opt/ml/detection/baseline/yolov5/data/trash.yaml', help='(optional) dataset.yaml path')
...
def inference(opt):
    ...
    for idx in range(4871):
        ...
        path = f"/opt/ml/detection/baseline/yolov5/runs/detect/{opt.name}/labels/{word}.txt"
    ...
    submission.to_csv(f'/opt/ml/Pstage/level2-object-detection-level2-cv-12/baseline/mmdetection/work_dirs/yolo/{opt.name}.csv',index=False)
