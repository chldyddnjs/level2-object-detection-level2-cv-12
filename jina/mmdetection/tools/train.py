...
def main():
    ...
    wandbname = str(args.config)
    wandbname = wandbname.split('/')[9][:-15] # wandbname 지정 (config 파일 _coco_custom.py)
    cfg.log_config = dict(
    interval=50,
    hooks=[
        ...
        dict(type='WandbLoggerHook',interval=1000,
            init_kwargs=dict(
                project= 'detection', # project name
                entity = 'yolo12', # entity name
                name = 'jina_'+ 'fold0_' +f'{wandbname}'+'_mixup', # run name
                config = dict(cfg)
            ),
            )
    ])
